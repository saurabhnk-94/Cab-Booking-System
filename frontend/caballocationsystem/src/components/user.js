        ...
        return ( <
            ...
            option > Select a User < /option> {
                this.state.user.map((item, index) => ( <
                    option key = { index }
                    value = { item.username } > { item.id } & nbsp; { item.username } <
                    /option>
                ))
            }
            ...
            div > < /div> {
                (this.state.username.length > 0 && this.state.username !== "Select a User") ? ( <
                    ...
                    onClick = {
                        () => this.bookingCab(this.state.username) } >
                    Book Cab <
                    /button>
                ) : null
            } { /* {console.log("Filtering", (this.state.rideDetails.filter(items => ((items.user === this.state.username && items.status !== "AC") || (items.user === this.state.username && items.status !== "RE"))).length > 0))} */ } { /* {console.log("ride", this.state.rideDetails)} */ } {
                this.state.rideDetails
                    ...
                    .map((item, index) => ( <
                        div key = { index }
                        className = "driver-on-going" > { /* {console.log("drivername", item.driver)} */ } <
                        h2 > You have requested
                        for the CAB < /h2> <
                        /div>
                    ))
            }

            {
                this.state.rideDetails
                    ...
                    .map((item, index) => ( <
                        ...
                        className = "driver-on-going" > { /* {console.log("drivername", item.driver)} */ } <
                        h2 >
                        You are on the CAB with { item.driver } < button onClick = {
                            () => this.endTrip(item.id) } > End Trip < /button> <
                        /h2> <
                        /div>
                    ))
            }

            {
                (this.state.username.length > 0 && this.state.username !== "Select a User") ? ( <
                    ...
                    tbody > {
                        ...
                        .map((item, index) => ( <
                            tr key = { index } > { /* {console.log("table content", item)} */ } <
                            td > { item.driver } < /td> <
